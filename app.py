...
if not st.session_state['logged_in']:
    ...
    def show_dashboard_page():
        # --- CSS: ปรับระยะห่างด้านบนใหม่ (ขยับลงมา 3rem ไม่ให้ชนขอบ) ---
        st.markdown("""
            <style>
               .block-container {
                    padding-top: 3rem; 
        ...
        # ตั้งค่า Theme กราฟให้ดูคลีนและทันสมัย
        ...
        with tab1:
            ...
            with c1:
                ...
                fig.update_layout(template=chart_template, legend=dict(orientation="h", y=1.1), margin=dict(l=10, r=10, t=30, b=10), height=350)
            ...
            with c2:
                ...
                fig2.update_layout(template=chart_template, legend=dict(orientation="h", y=1.1), margin=dict(l=10, r=10, t=30, b=10), height=350)
            ...
            fig_adr.update_traces(line_color='#FF8700', line_width=4) # เส้นหนาขึ้นเห็นชัดๆ
            fig_adr.update_layout(height=300)
        ...
        with tab2:
            ...
            with c3:
                ...
                fig_pie = px.pie(res_rev, values='Price', names='Reservation', hole=0.4, template=chart_template, color_discrete_sequence=px.colors.qualitative.Pastel)
                fig_pie.update_traces(textposition='inside', textinfo='percent+label')
                fig_pie.update_layout(margin=dict(t=30, b=20), height=350, showlegend=False)
            ...
            with c4:
                ...
                fig_bar = px.bar(m_res, x='M_Name', y='Count', color='Reservation', template=chart_template)
                fig_bar.update_layout(margin=dict(t=30, b=20), height=350)
            ...
            fig_bar_h = px.bar(chan_adr, x='Reservation', y='ADR', color='ADR', color_continuous_scale='Viridis', title="Channel Value (ADR)", template=chart_template)
        ...
        with tab3:
            ...
            with c5:
                ...
                fig_room = px.bar(mt_room, x='M_Name', y='Price', color=group_col, title="Room Revenue", template=chart_template)
                st.plotly_chart(fig_room, use_container_width=True)
            with c6:
                heatmap_data = df_filtered.groupby([group_col, 'Reservation']).size().unstack(fill_value=0)
                fig_heat = px.imshow(heatmap_data, text_auto=True, aspect="auto", color_continuous_scale='Blues', title="Room vs Channel", template=chart_template)
            ...
            with c7: 
                fig_pie2 = px.pie(day_rev, values='Price', names='DayType', hole=0.4, title="Wkday vs Wkend", template=chart_template)
                st.plotly_chart(fig_pie2, use_container_width=True)
            with c8:
                day_avg = df_filtered.groupby('DayType')['Price'].mean().reset_index()
                fig_bar2 = px.bar(day_avg, x='DayType', y='Price', title="Avg Price", color='DayType', template=chart_template)
