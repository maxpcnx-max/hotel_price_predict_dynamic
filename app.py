...
if not st.session_state['logged_in']:
    ...
    def show_manage_data_page():
        ...
        with tab_master:
            ...
            st.markdown("#### 1. จัดการห้องพัก (Room Management)")
            st.caption("ตั้งชื่อห้อง: จับคู่ 'เลขห้อง' (101) -> 'ชื่อห้อง' (Deluxe)")
            ...
            if st.button("💾 บันทึกชื่อห้อง (Save Rooms)"):
                try:
            ...
            # 2. Base Price
            st.divider()
            st.markdown("#### 2. จัดการราคา (Price Management)")
            st.caption("ราคากลาง: ใช้สำหรับให้ AI คำนวณเป็นฐาน (ไม่ควรต่ำกว่านี้)")
            ...
            edited_prices_df = st.data_editor(
                ...
                use_container_width=True,
                column_config={
                    "Room Type": st.column_config.TextColumn("Room Name (ชื่อต้องตรงกับข้อ 1)"),
            ...
            edited_res_master = st.data_editor(
                ...
                num_rows="dynamic",
                use_container_width=True,
                key="res_master_editor",
                column_config={
                    "Reservation": st.column_config.TextColumn("Original Name (ชื่อเดิมในไฟล์)", disabled=True),
                    "Reservation_Type": st.column_config.TextColumn("New Name (ชื่อใหม่ที่ต้องการแสดง)")
                }
            )

            if st.button("💾 บันทึกช่องทาง (Save Channels)"):
                try:
                    edited_res_master.to_csv(RES_FILE, index=False)
                    st.cache_data.clear()
                    st.success("✅ บันทึกช่องทางการขายเรียบร้อย!")
                    time.sleep(1)
                    st.rerun()
                except Exception as e: st.error(f"Error: {e}")
