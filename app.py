...
if not st.session_state['logged_in']:
    ...
    def show_manage_data_page():
        ...
        with tab_trans:
            ...
            if up_file is not None:
                try:
                    # 1. อ่านไฟล์มาเก็บในตัวแปร new_data ก่อน
                    up_file.seek(0)
                    new_data = pd.read_csv(up_file)

                    # 2. แสดงตัวอย่างข้อมูล (Preview)
                    st.markdown(f"##### 📄 ตรวจสอบข้อมูลก่อนบันทึก ({len(new_data)} รายการ)")
                    st.dataframe(new_data, use_container_width=True, height=300) # แสดงตาราง สูง 300px

                    # 3. ปุ่มบันทึก (ย้ายมาอยู่ใต้ตาราง)
                    col_save, _ = st.columns([1, 4])
                    with col_save:
                        if st.button("💾 ยืนยันการบันทึก", type="primary", use_container_width=True):
                            # ส่ง new_data ที่อ่านไว้แล้วไปบันทึกได้เลย ไม่ต้องอ่านซ้ำ
                            if save_data_robust(new_data, mode='append'):
                                st.success(f"✅ บันทึกเรียบร้อย! ({len(new_data)} รายการ) อย่าลืมกด Retrain Model ที่แท็บที่ 3")
                                time.sleep(2)
                                st.rerun()
    ...
    def show_about_page():
        ...
        with c2:
            ...
            st.divider()
            st.info("วิทยานิพนธ์: การพัฒนาระบบสนับสนุนการตัดสินใจเพื่อการพยากรณ์ราคาแบบพลวัตสำหรับธุรกิจโรงแรม โดยใช้เทคนิคเหมืองข้อมูล ")
