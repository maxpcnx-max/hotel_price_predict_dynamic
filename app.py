...
# ==========================================================
# 4. DATA HANDLING (SMART DATE & ROOM NAME FIX)
# ==========================================================
...
def normalize_room_id(val):
    """ฟังก์ชันช่วยแปลงรหัสห้องให้ตรงกัน (แก้ปัญหา 1 vs 1.0)"""
    try:
        val_float = float(val)
...
@st.cache_data
def load_data():
    """โหลดข้อมูลและ Merge ชื่อห้องจากไฟล์ CSV เสมอ"""
    # 1. โหลดข้อมูล Transaction
    ...
    try:
        ...
        # เตรียมคอลัมน์ Room ให้เป็น format มาตรฐาน
        if 'Room' in df.columns:
            df['Room'] = df['Room'].apply(normalize_room_id)

        # 2. โหลดข้อมูล Master Room Type (room_type.csv)
        if os.path.exists(ROOM_FILE):
            try:
                room_master = pd.read_csv(ROOM_FILE)
                # เตรียมคอลัมน์ Room ของ Master
                if 'Room' in room_master.columns:
                    room_master['Room'] = room_master['Room'].apply(normalize_room_id)
                
                # เลือกเฉพาะคอลัมน์ที่จำเป็น
                cols_to_use = ['Room']
                if 'Room_Type' in room_master.columns: cols_to_use.append('Room_Type')
                elif 'Target_Room_Type' in room_master.columns: cols_to_use.append('Target_Room_Type')
        ...
        # 4. Fallback: ถ้าหาชื่อไม่เจอจริงๆ ให้ใช้เลขห้องไปก่อน
    ...
    try:
        ...
        if 'Target_Room_Type' in df_clean.columns:
             ...
             if not valid_rooms and os.path.exists(ROOM_FILE):
                  try:
                      rf = pd.read_csv(ROOM_FILE)
                      if 'Room_Type' in rf.columns: valid_rooms = set(rf['Room_Type'].unique())
                  except: pass
...
if not st.session_state['logged_in']:
    ...
    def show_dashboard_page():
        ...
        # ใช้ Target_Room_Type (ชื่อห้อง) แทน Room (เลขห้อง)
    ...
    def show_manage_data_page():
        st.title("📥 ระบบจัดการฐานข้อมูล (Integrated)")
        
        tab_trans, tab_master, tab_train = st.tabs(["📝 ข้อมูลการจอง (Transactions)", "⚙️ ข้อมูลห้องพัก (Master Data)", "🚀 อัปเดตโมเดล (Retrain)"])
        ...
        with tab_trans:
            ...
            edited_df = st.data_editor(
                ...
                column_config={
                    ...
                    "Room": st.column_config.TextColumn("Room (เลขห้อง)"),
        ...
        with tab_master:
            ...
            with c_master1:
                st.subheader("1. ตั้งชื่อห้อง (Room Mapping)")
                st.caption("จับคู่ 'เลขห้อง' กับ 'ชื่อห้อง' สำหรับแสดงใน Dashboard")
                
                if os.path.exists(ROOM_FILE):
                    df_room_master = pd.read_csv(ROOM_FILE)
                    if 'Room' in df_room_master.columns: 
                        df_room_master['Room'] = df_room_master['Room'].astype(str)
                else:
                    df_room_master = pd.DataFrame(columns=['Room', 'Room_Type'])

                edited_room_master = st.data_editor(
                    df_room_master,
                    num_rows="dynamic",
                    use_container_width=True,
                    key="room_master_editor",
                    column_config={
                        "Room": st.column_config.TextColumn("Room ID (เลขห้อง)", help="เช่น 101, 4.1"),
                        "Room_Type": st.column_config.TextColumn("Room Name (ชื่อห้อง)", help="เช่น Deluxe, Grand Suite")
                    }
                )

                if st.button("💾 บันทึกชื่อห้อง (Save Mapping)"):
                    try:
                        edited_room_master.to_csv(ROOM_FILE, index=False)
                        st.cache_data.clear()
                        st.success("✅ บันทึกชื่อห้องเรียบร้อย!")
                        time.sleep(1)
                        st.rerun()
                    except Exception as e:
                        st.error(f"Error: {e}")
            ...
            with c_master2:
                st.subheader("2. ราคาฐาน (Base Price)")
                st.caption("กำหนดราคากลางสำหรับให้ AI ใช้คำนวณ")
                ...
                edited_prices_df = st.data_editor(
                    ...
                    use_container_width=True,
                    column_config={
                        "Room Type": st.column_config.TextColumn("Room Name (ชื่อต้องตรงกับฝั่งซ้าย)"),
                        "Base Price": st.column_config.NumberColumn("Base Price (THB)", format="%d THB")
                    },
                    key="base_price_editor"
                )
                
                if st.button("💾 บันทึกราคาฐาน (Save Prices)"):
    ...
    # ==========================================================
    # 7. SIDEBAR & PAGE ROUTING (MODIFIED)
    # ==========================================================
    with st.sidebar:
